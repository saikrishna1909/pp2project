...
public class TeacherPerSubjectDaoImpl implements TeacherPerSubjectDao {
	...
	public List<Subject> getAllSubjects() throws SQLException {
		...
		String sql = "select classes.class_id, classes.class_name, " + "subjects.subject_id, subjects.subject_name, "
	...
	public SQLState updateOneSubject(JSONObject jsonObject) {
		...
		try {
			...
			System.out
					.println("----> jsonObject.getInt(\"teacherId\"): " + ((ResultSet) jsonObject).getInt("teacherId"));
			System.out.println("----> jsonObject.getString(\"subjectName\"): "
					+ ((ResultSet) jsonObject).getString("subjectName"));
			System.out.println(
					"----> jsonObject.getString(\"subjectId\"): " + ((ResultSet) jsonObject).getString("subjectId"));
	...
	public SQLState saveNewSubject(JSONObject jsonObject) {
		...
		try {
			// SQL String
			String insert = " insert into subjects (subject_id, teacher_id, subject_name, teacher_name) values (?, ?, ?,?) ";
			System.out.println("----> insert: " + insert);
			PreparedStatement st = this.conn.prepareStatement(insert);

			st.setString(0, jsonObject.getString("subjectId"));
			st.setInt(1, jsonObject.getInt("teacherId"));
			st.setString(2, jsonObject.getString("subjectName"));
			st.setString(3, jsonObject.getString("teacherName"));
